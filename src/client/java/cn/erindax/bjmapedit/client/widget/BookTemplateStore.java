...
public final class BookTemplateStore {
	...
	public static boolean add(Entry entry) {
		...
		if (entry.pages == null) entry.pages = new ArrayList<>();
	...
	private static void load() {
		...
		FOLDERS.clear();
		try {
			String json = TemplateFiles.read("book_templates.json");
			if (json == null) return;
			...
			if (data.templates != null) {
				for (Entry e : data.templates) {
					if (e == null) continue;
					if (e.pages == null) e.pages = new ArrayList<>();
			...
			BjMapEdit.LOGGER.warn("Failed to load book templates", e);
		}
	}
	...
	private static void save() {
		try {
			...
			TemplateFiles.write("book_templates.json", GSON.toJson(data));
		} catch (Exception e) {
			BjMapEdit.LOGGER.warn("Failed to save book templates", e);
		}
	}
