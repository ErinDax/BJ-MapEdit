...
public final class EntityTemplateStore {
	...
	public static boolean add(Entry entry) {
		...
		if (entry.keys == null) entry.keys = new ArrayList<>();
	...
	private static void load() {
		...
		FOLDERS.clear();
		try {
			String json = TemplateFiles.read("entity_templates.json");
			if (json == null) return;
			...
			BjMapEdit.LOGGER.warn("Failed to load entity templates", e);
		}
	}
	...
	private static void save() {
		try {
			...
			TemplateFiles.write("entity_templates.json", GSON.toJson(data));
		} catch (Exception e) {
			BjMapEdit.LOGGER.warn("Failed to save entity templates", e);
		}
	}
